...
df_fraud = df_claims.alias("c").join(
    df_policies.alias("p").select("policy_id", "annual_premium", "coverage_amount", "state_code"), "policy_id", "left"
)

# Calculate fraud indicators
df_fraud = (
    df_fraud.withColumn("late_reporting_flag", F.col("days_to_report") > 7)
...
df_fraud = df_fraud.withColumn("customer_total_claims", F.count("*").over(window_customer)).withColumn(
    "frequent_claimant_flag", F.col("customer_total_claims") > 3
)
...
df_fraud = df_fraud.withColumn(
    ...
    F.when(
        (F.col("claim_type") == "Auto") & (F.col("claimed_amount") > 20000) & (F.col("days_to_report") > 5), True
    ).otherwise(False),
)
...
fraud_indicator_cols = [
    ...
    "location_mismatch_flag",
    "frequent_claimant_flag",
    "medical_billing_anomaly_flag",
    "suspicious_injury_pattern_flag",
]

df_fraud = df_fraud.withColumn(
    "total_fraud_indicators", sum([F.when(F.col(c), 1).otherwise(0) for c in fraud_indicator_cols])
)
...
df_fraud = df_fraud.withColumn("behavioral_score", F.round(F.expr("rand() * 30 + 20 + total_fraud_indicators * 5"), 2))
...
df_fraud = (
    df_fraud.withColumn(
        "provider_id",
        F.when(
            F.col("claim_type").isin(["Auto", "Property"]),
            F.concat(F.lit("PROV"), F.lpad(F.expr("cast(rand() * 1000 as int)"), 4, "0")),
        ).otherwise(F.lit(None)),
    )
    .withColumn(
        "provider_name",
        F.when(F.col("provider_id").isNotNull(), F.concat(F.lit("Provider "), F.col("provider_id"))).otherwise(
            F.lit(None)
        ),
    )
...
df_fraud = df_fraud.withColumn(
    ...
    F.when(F.col("annual_premium") > 0, F.round(F.col("claimed_amount") / F.col("annual_premium"), 4)).otherwise(
        F.lit(0)
    ),
)
...
df_fraud = (
    ...
    .withColumn(
        ...
        F.when(
            F.col("siu_referral"),
            F.element_at(
                F.array([F.lit("Pending"), F.lit("In Progress"), F.lit("Completed")]),
    ...
    .withColumn(
        ...
        F.when(
            F.col("investigation_status") == "Completed",
            F.element_at(
                F.array([F.lit("Confirmed Fraud"), F.lit("Not Fraud"), F.lit("Suspicious")]),
                F.when(F.col("overall_fraud_score") > 70, 1).when(F.col("overall_fraud_score") > 50, 3).otherwise(2),
            ),
        ).otherwise(F.lit(None)),
    )
    .withColumn(
        "investigator_id",
        F.when(
            F.col("siu_referral"), F.concat(F.lit("INV"), F.lpad(F.expr("cast(rand() * 50 as int)"), 3, "0"))
        ).otherwise(F.lit(None)),
    )
)
...
df_fraud = df_fraud.withColumn(
    "estimated_exposure", F.round(F.col("claimed_amount") * F.col("ml_fraud_prediction"), 2)
...
# COMMAND ----------
df_fraud = (
    df_fraud.withColumn("model_version", F.lit("v1.0.0"))
...
# COMMAND ----------
df_fraud = (
    df_fraud.withColumn("analysis_date", F.current_date())
...
final_columns = [
    ...
    # Customer history
    "customer_total_claims_history",
    "customer_siu_referrals",
    "customer_denied_claims",
    "customer_fraud_score",
    # Provider
    "provider_id",
    "provider_name",
    "provider_fraud_history",
...
df_fraud_final.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(table_name)

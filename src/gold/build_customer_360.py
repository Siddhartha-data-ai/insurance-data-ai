...
df_policy_metrics = df_policies.groupBy("customer_id").agg(
    ...
    F.sum(F.when(F.col("policy_status") == "Active", F.col("annual_premium")).otherwise(0)).alias(
...
# COMMAND ----------
df_customer_360 = df_customer_base.join(df_policy_metrics, "customer_id", "left").join(
    df_claims_metrics, "customer_id", "left"
)
...
df_customer_360 = df_customer_360.withColumn(
    "loss_ratio",
    F.when(
        F.col("total_lifetime_premium") > 0, F.round(F.col("total_claims_paid") / F.col("total_lifetime_premium"), 4)
    ).otherwise(F.lit(0)),
)

# Customer lifetime value (simple calculation)
df_customer_360 = df_customer_360.withColumn(
    "customer_lifetime_value", F.round(F.col("total_lifetime_premium") - F.col("total_claims_paid"), 2)
)
...
# Profitability
df_customer_360 = df_customer_360.withColumn(
    "profitability", F.round(F.col("total_lifetime_premium") * 0.7 - F.col("total_claims_paid"), 2)
)
...
df_customer_360 = df_customer_360.withColumn(
    ...
    F.round(
        (
            ...
            + F.when(F.col("days_since_last_claim").isNull(), 0.0)
            .when(F.col("days_since_last_claim") < 90, 0.10)
            .otherwise(0.0)
        ),
        4,
    ),
...
df_customer_360 = df_customer_360.withColumn(
    "churn_risk_category",
    F.when(F.col("churn_risk_score") > 0.60, "High").when(F.col("churn_risk_score") > 0.30, "Medium").otherwise("Low"),
)

# Retention probability (inverse of churn risk)
df_customer_360 = df_customer_360.withColumn("retention_probability", F.round(1.0 - F.col("churn_risk_score"), 4))
...
df_customer_360 = (
    df_customer_360.withColumn("last_contact_date", F.date_sub(F.current_date(), F.expr("cast(rand() * 90 as int)")))
...
# COMMAND ----------
df_customer_360 = (
    df_customer_360.withColumn("snapshot_date", F.current_date())
...
df_customer_360.write.format("delta").mode("overwrite").option("overwriteSchema", "true").saveAsTable(table_name)

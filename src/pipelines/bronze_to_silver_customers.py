...
# COMMAND ----------
@dlt.table(
    name="customer_raw_stream",
    comment="Streaming view of raw customer data"
)
...
@dlt.table(
    ...
    table_properties={
        "quality": "silver",
        "pipelines.autoOptimize.zOrderCols": "customer_id,state_code"
    }
)
...
def customer_validated():
    ...
    return (
        ...
        .withColumn("customer_tenure_months", 
                    F.floor(F.months_between(F.current_date(), F.col("customer_since_date"))))
...
@dlt.table(
    ...
    comment="Customer data enriched with reference data"
)
def customer_enriched():
    ...
    customers = customers.withColumn("assigned_agent_name", 
                                     F.concat(F.lit("Agent "), F.col("assigned_agent_id")))
...
dlt.apply_changes(
    ...
    track_history_column_list=["first_name", "last_name", "email", "phone", "address_line1", 
                                "city", "state_code", "customer_status", "customer_segment"]
)
...
# COMMAND ----------
@dlt.table(
    name="customer_quality_metrics",
    comment="Data quality metrics for customer pipeline"
)

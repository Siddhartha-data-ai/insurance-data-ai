...
# COMMAND ----------
@dlt.table(name="agent_raw_stream", comment="Streaming view of raw agent data")
def agent_raw_stream():
    return spark.readStream.format("delta").option("readChangeFeed", "true").table(f"{bronze_catalog}.agents.agent_raw")
...
@dlt.table(
    ...
    table_properties={"quality": "silver", "pipelines.autoOptimize.zOrderCols": "agent_id,region_code"},
)
...
def agent_validated():
    ...
    return (
        df.withColumn("years_of_experience", F.floor(F.months_between(F.current_date(), F.col("hire_date")) / 12))
        .withColumn("agent_tenure_months", F.floor(F.months_between(F.current_date(), F.col("hire_date"))))
        .withColumn(
            "performance_tier",
        ...
        .withColumn("is_active_agent", F.when(F.col("agent_status") == "Active", True).otherwise(False))
        .withColumn(
            "commission_tier",
        ...
        .withColumn("total_licenses", F.size(F.split(F.col("licensed_states"), ",")))
...
# COMMAND ----------
@dlt.table(name="agent_enriched", comment="Agent data enriched with hierarchy and territory")
def agent_enriched():
    ...
    agents = (
        agents.withColumn(
            "team_lead_name",
            F.when(F.col("team_lead_id").isNotNull(), F.concat(F.lit("Team Lead "), F.col("team_lead_id"))).otherwise(
                None
            ),
        )
        .withColumn(
            ...
            F.when(
                F.col("regional_manager_id").isNotNull(),
                F.concat(F.lit("Regional Manager "), F.col("regional_manager_id")),
            ).otherwise(None),
        )
        .withColumn(
            "vp_name", F.when(F.col("vp_id").isNotNull(), F.concat(F.lit("VP "), F.col("vp_id"))).otherwise(None)
        )
    )

    # Add performance metrics
    agents = (
        agents.withColumn("ytd_policies_sold", F.coalesce(F.col("ytd_policies_sold"), F.lit(0)))
        ...
        .withColumn(
            "avg_policy_size",
            F.when(F.col("ytd_policies_sold") > 0, F.col("ytd_premium_written") / F.col("ytd_policies_sold")).otherwise(
                0
            ),
        )
    )
    ...
    agents = agents.withColumn("primary_territory", F.element_at(F.split(F.col("territory_code"), "-"), 1)).withColumn(
        "territory_type",
...
dlt.create_streaming_table(
    ...
    table_properties={"quality": "silver", "delta.enableChangeDataFeed": "true", "hierarchy_enabled": "true"},
...
dlt.apply_changes(target="agent_dim", source="agent_updates", keys=["agent_id"], sequence_by=F.col("updated_timestamp"))
...
# COMMAND ----------
@dlt.table(name="agent_quality_metrics", comment="Data quality metrics for agent pipeline")

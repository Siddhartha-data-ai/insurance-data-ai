...
# COMMAND ----------
@dlt.table(
    name="payment_raw_stream",
    comment="Streaming view of raw payment data"
)
...
@dlt.table(
    ...
    table_properties={
        "quality": "silver",
        "pipelines.autoOptimize.zOrderCols": "payment_id,policy_id,customer_id"
    }
)
...
def payment_validated():
    ...
    return (
        df
        .withColumn("payment_status_code",
        ...
        .withColumn("transaction_fee",
                    F.when(F.col("payment_method") == "Credit Card", F.col("payment_amount") * 0.029 + 0.30)
                     ...
                     .otherwise(0))
        .withColumn("net_payment_amount",
                    F.col("payment_amount") - F.col("transaction_fee"))
        .withColumn("is_late_payment",
                    F.when(F.col("days_overdue") > 0, True).otherwise(False))
        .withColumn("late_fee",
                    F.when(F.col("days_overdue") > 30, 50.00)
                     .when(F.col("days_overdue") > 15, 25.00)
                     .when(F.col("days_overdue") > 0, 10.00)
                     .otherwise(0))
        .withColumn("payment_size_category",
...
# COMMAND ----------
@dlt.table(
    name="payment_enriched",
    comment="Payment data enriched with business metrics"
)
    ...
    payments = (payments
        ...
        .withColumn("payment_day_name",
                    F.when(F.dayofweek(F.col("payment_date")) == 1, "Sunday")
                     ...
                     .otherwise("Saturday"))
        .withColumn("is_weekend_payment",
                    F.when(F.dayofweek(F.col("payment_date")).isin([1, 7]), True).otherwise(False))
    )
    ...
    payments = (payments
        .withColumn("payment_type",
                    F.when(F.col("payment_reason").like("%Premium%"), "Premium Payment")
                     ...
                     .otherwise("Other"))
        .withColumn("is_auto_payment",
                    F.when(F.col("payment_method") == "Auto Pay", True).otherwise(False))
        .withColumn("is_recurring",
                    F.when(F.col("payment_frequency").isNotNull(), True).otherwise(False))
    )
    
    # Add processing attributes
    payments = (payments
        .withColumn("processor_name",
                    F.when(F.col("payment_processor_id").isNotNull(),
                           F.concat(F.lit("Processor "), F.col("payment_processor_id")))
                     .otherwise("Direct"))
        .withColumn("gateway_name",
                    F.when(F.col("payment_gateway_id").isNotNull(),
                           F.concat(F.lit("Gateway "), F.col("payment_gateway_id")))
                     .otherwise("Internal"))
    )
...
# COMMAND ----------
@dlt.table(
    name="payment_quality_metrics",
    comment="Data quality metrics for payment pipeline"
)

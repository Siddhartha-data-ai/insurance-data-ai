...
# COMMAND ----------
@dlt.table(
    name="policy_raw_stream",
    comment="Streaming view of raw policy data"
)
def policy_raw_stream():
    return (
        spark.readStream
            ...
            .table(f"{bronze_catalog}.policies.policy_raw")
    )
...
@dlt.table(
    ...
    table_properties={
        "quality": "silver",
        "pipelines.autoOptimize.zOrderCols": "policy_id,customer_id,policy_type"
    }
)
    ...
    return (
        ...
        .withColumn("policy_status_code",
                    F.when(F.col("policy_status") == "Active", 1)
                     ...
                     .otherwise(0))
        .withColumn("days_in_force", 
                    F.datediff(F.current_date(), F.col("effective_date")))
        .withColumn("earned_premium",
                    F.when(F.col("policy_status") == "Active", 
                           F.col("annual_premium") * F.datediff(F.current_date(), F.col("effective_date")) / 365)
                     .otherwise(0))
        .withColumn("unearned_premium",
                    F.col("annual_premium") - F.col("earned_premium"))
        .withColumn("days_overdue",
                    F.when(F.col("premium_payment_status") == "Overdue",
                           F.datediff(F.current_date(), F.col("effective_date")))
                     .otherwise(0))
        .withColumn("cancellation_category",
                    F.when(F.col("cancellation_reason").like("%non-payment%"), "Non-Payment")
                     ...
                     .otherwise("Other"))
        .withColumn("policy_value_score",
                    (F.col("annual_premium") * 0.4 + F.col("coverage_amount") / 100000 * 0.6))
...
# COMMAND ----------
@dlt.table(
    name="policy_enriched",
    comment="Policy data enriched with reference data"
)
def policy_enriched():
    policies = dlt.read_stream("policy_validated")
    
    # Add state name
    states = spark.table(f"{silver_catalog}.master_data.state_dim") if spark.catalog.tableExists(f"{silver_catalog}.master_data.state_dim") else None
    ...
    # Add agent names
    policies = (policies
        .withColumn("writing_agent_name", F.concat(F.lit("Agent "), F.col("writing_agent_id")))
    ...
    # Add product name
    policies = policies.withColumn("product_name", 
...
# COMMAND ----------
@dlt.table(
    name="policy_quality_metrics",
    comment="Data quality metrics for policy pipeline"
)

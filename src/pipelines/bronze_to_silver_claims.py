...
# COMMAND ----------
@dlt.table(
    name="claim_raw_stream",
    comment="Streaming view of raw claims data"
)
def claim_raw_stream():
    return (
        spark.readStream
            ...
            .table(f"{bronze_catalog}.claims.claim_raw")
    )
...
@dlt.table(
    ...
    table_properties={
        "quality": "silver",
        "pipelines.autoOptimize.zOrderCols": "claim_id,policy_id,customer_id"
    }
)
...
def claim_validated():
    ...
    return (
        ...
        .withColumn("days_to_report", 
                    F.datediff(F.col("reported_date"), F.col("loss_date")))
        .withColumn("days_to_close",
                    F.when(F.col("is_closed"), F.datediff(F.col("closed_date"), F.col("reported_date")))
                     .otherwise(F.datediff(F.current_date(), F.col("reported_date"))))
        .withColumn("outstanding_amount",
                    F.col("claimed_amount") - F.col("paid_amount"))
        ...
        .withColumn("payment_ratio",
                    F.when(F.col("claimed_amount") > 0, F.col("paid_amount") / F.col("claimed_amount"))
                     .otherwise(0))
        .withColumn("is_suspicious",
                    F.when((F.col("fraud_score") > 70) | 
                           (F.col("days_to_report") > 30) |
                           (F.col("claimed_amount") > 100000), True)
                     .otherwise(False))
...
@dlt.table(
    name="claim_enriched",
    comment="Claims enriched with fraud indicators and reference data"
)
def claim_enriched():
    ...
    claims = (claims
        .withColumn("excessive_amount_flag",
                    F.when(F.col("claimed_amount") > 100000, 1).otherwise(0))
        .withColumn("late_reporting_flag",
                    F.when(F.col("days_to_report") > 30, 1).otherwise(0))
        .withColumn("weekend_incident_flag",
                    F.when(F.dayofweek(F.col("loss_date")).isin([1, 7]), 1).otherwise(0))
        .withColumn("multiple_claims_flag",
                    F.when(F.col("total_claims") > 3, 1).otherwise(0))
        .withColumn("new_policyholder_flag",
                    F.when(F.col("policy_tenure_months") < 6, 1).otherwise(0))
        .withColumn("round_amount_flag",
                    F.when(F.col("claimed_amount") % 1000 == 0, 1).otherwise(0))
    )
    
    # Calculate total fraud indicators
    claims = claims.withColumn("total_fraud_indicators",
    ...
    claims = claims.withColumn("enhanced_fraud_score",
        (F.col("fraud_score") * 0.7 + F.col("total_fraud_indicators") * 5))
    
    # Add adjuster and provider names
    claims = (claims
        .withColumn("adjuster_name", F.concat(F.lit("Adjuster "), F.col("assigned_adjuster_id")))
        .withColumn("provider_name", F.concat(F.lit("Provider "), F.col("service_provider_id")))
    )
...
# COMMAND ----------
@dlt.table(
    name="claim_quality_metrics",
    comment="Data quality metrics for claims pipeline"
)

...
if not df_runs.empty:
    ...
    display_df = recent_10[['job_name', 'status', 'duration_minutes', 'start_time', 'total_tasks', 'failed_tasks']].copy()
...
if not df_runs.empty:
    ...
    job_stats = df_runs.groupby('job_name').agg({
        'run_id': 'count',
...
if not df_runs.empty and not df_runs[df_runs['duration_minutes'].notna()].empty:
    ...
    duration_stats = df_runs[df_runs['duration_minutes'].notna()].groupby('job_name').agg({
        'duration_minutes': ['mean', 'min', 'max', 'count']
    }).round(1)

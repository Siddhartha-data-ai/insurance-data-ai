...
from pyspark.sql.functions import col, sum as spark_sum, count, avg, max as spark_max, min as spark_min, round as spark_round, current_timestamp
...
if all_storage_data:
    ...
    # Add layer column
    df_storage['layer'] = df_storage['catalog'].apply(lambda x: 'Bronze' if 'bronze' in x else 'Silver' if 'silver' in x else 'Gold')
...
if not df_storage.empty:
    ...
    layer_summary = df_storage.groupby('layer').agg({
        'size_gb': 'sum',
        'monthly_storage_cost': 'sum'
    }).reset_index()
...
if recommendations:
    ...
    total_potential = sum([float(rec['potential_savings'].replace('$', '').replace(',', '').split('/')[0]) 
...
print("\nTo save the report:")
print(f"# dbutils.fs.put('/tmp/cost_report_{ENVIRONMENT}_{datetime.now().strftime(\"%Y%m%d\")}.json', report_json, True)")

...
def apply_scd_type2(source_df, target_table, business_key, tracking_columns, schema_name):
    ...
    if table_exists:
        ...
        has_scd_columns = all(col in existing_columns for col in ["is_current", "effective_start_date", "record_version"])
    ...
    if table_exists and has_scd_columns:
        ...
        for col_name in tracking_columns:
            change_conditions.append(f"(source.{col_name} <> target.{col_name} OR (source.{col_name} IS NULL AND target.{col_name} IS NOT NULL) OR (source.{col_name} IS NOT NULL AND target.{col_name} IS NULL))")
        
        change_condition = " OR ".join(change_conditions)
        
        # Get current records that will be closed
        df_to_close = df_existing.alias("target") \
        ...
        df_updates = df_new.alias("source") \
            .join(df_existing.alias("target").filter("is_current = true"), 
                  [business_key] if isinstance(business_key, str) else business_key, "left") \
            .where(f"target.{business_key} IS NULL OR ({change_condition})") \
            .select("source.*") \
            .withColumn("record_version", 
                       when(col(f"target.{business_key}").isNull(), lit(1))
                       .otherwise(col("target.record_version") + 1))
...
df_customers_validated = df_customers \
...
df_customers_silver = df_customers_validated \
    ...
    .withColumn("customer_tenure_months", 
                F.floor(F.months_between(current_date(), col("customer_since_date")))) \
...
tracking_cols = ["first_name", "last_name", "email", "phone", "address_line1", "city", 
                 "state_code", "customer_status", "customer_segment", "annual_income"]
...
# Apply data quality filters
df_policies_validated = df_policies \
...
# Add transformations
df_policies_silver = df_policies_validated \
    .withColumn("policy_age_days", F.datediff(current_date(), col("effective_date"))) \
    ...
    .withColumn("annual_premium_tier",
                when(col("annual_premium") >= 5000, "High")
                .when(col("annual_premium") >= 2000, "Medium")
                .otherwise("Low")) \
...
# Add transformations
df_claims_silver = df_claims_validated \
    .withColumn("claim_age_days", F.datediff(current_date(), col("report_date"))) \
    ...
    .withColumn("claim_amount_tier",
                when(col("claimed_amount") >= 50000, "Large")
                .when(col("claimed_amount") >= 10000, "Medium")
                .otherwise("Small")) \
    ...
    .withColumn("settlement_ratio", 
                when(col("claimed_amount") > 0, col("paid_amount") / col("claimed_amount"))
                .otherwise(lit(None))) \
    .withColumn("load_timestamp", current_timestamp()) \
...
display(
    ...
    .select("customer_id", "full_name", "email", "customer_status", "is_current", 
            "effective_start_date", "effective_end_date", "record_version")

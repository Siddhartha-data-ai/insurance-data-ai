...
df_policy_analysis = df_policies.alias("p") \
...
df_policy_analysis = df_policy_analysis \
    .withColumn("claim_count", F.coalesce(col("claim_count"), lit(0))) \
...
df_policy_analysis = df_policy_analysis.withColumn(
    "loss_ratio",
    when((col("annual_premium") > 0) & (col("policy_age_days") > 0), 
         col("total_paid") / (col("annual_premium") * col("policy_age_days") / 365.0)).otherwise(0)
)
...
df_customer_value = df_customer_value.withColumn(
    "expected_years",
    when(col("age_years") < 35, 10)
    .when(col("age_years") < 50, 8)
    .when(col("age_years") < 65, 6)
    .otherwise(4)
).withColumn(
...
df_policy_analysis = df_policy_analysis.join(
    df_customer_value.select("customer_id", "estimated_clv", "retention_probability"),
    "customer_id",
    "left"
)
...
if has_churn_data:
    df_policy_analysis = df_policy_analysis.join(
        df_churn.select("customer_id", 
                       col("churn_probability").alias("churn_risk"),
                       col("churn_risk_category")),
        "customer_id",
        "left"
    )
    df_policy_analysis = df_policy_analysis.withColumn(
        "adjusted_retention_prob",
        spark_round((1 - col("churn_risk") / 100) * col("retention_probability"), 4)
    )
else:
    df_policy_analysis = df_policy_analysis \
        .withColumn("churn_risk", lit(20)) \
...
df_policy_analysis_valid = df_policy_analysis.filter(
    ...
    (col("policy_age_days") > 0) &
    (col("market_avg_premium").isNotNull())
)

df_recommendations = df_policy_analysis_valid.withColumn(
    ...
    when(
        ...
        col("retention_adjustment") > 1.25,
        1.25
    ).when(
        col("loss_ratio_adjustment") * 
        col("risk_adjustment") * 
        col("market_adjustment") * 
        col("retention_adjustment") < 0.75,
        0.75
    ).otherwise(
        col("loss_ratio_adjustment") * 
        col("risk_adjustment") * 
    ...
    "recommended_premium",
    spark_round(col("annual_premium") * col("combined_adjustment"), 2)
).withColumn(
    "premium_change_amount",
    spark_round(col("recommended_premium") - col("annual_premium"), 2)
).withColumn(
    "premium_change_percent",
    spark_round((col("combined_adjustment") - 1) * 100, 2)
).withColumn(
    "annual_revenue_impact",
    spark_round(col("premium_change_amount") * col("adjusted_retention_prob"), 2)
).withColumn(
    ...
    "rationale",
    when(col("recommendation_category") == "Increase",
         F.concat(
             lit("Loss ratio: "), spark_round(col("loss_ratio"), 2),
             lit(", Risk: "), spark_round(col("risk_score"), 0)
         ))
    .when(col("recommendation_category") == "Decrease",
         F.concat(
             lit("Retention risk: "), spark_round(col("churn_risk"), 0),
             lit("%, CLV: $"), spark_round(col("estimated_clv"), 0)
         ))
...
final_recommendations = df_recommendations.select(
    "policy_id", "policy_number", "customer_id", "policy_type", "state_code",
    "annual_premium", "recommended_premium", "premium_change_amount", "premium_change_percent",
    "recommendation_category", "implementation_priority", "loss_ratio", "risk_score",
    "estimated_clv", "churn_risk", "adjusted_retention_prob", "annual_revenue_impact",
    "rationale", "recommendation_date"
)

print(f"✅ Generated recommendations for {final_recommendations.count():,} policies")

print("\n📊 Recommendation Distribution:")
final_recommendations.groupBy("recommendation_category", "implementation_priority") \
    .count().orderBy("recommendation_category", "implementation_priority").show()
...
display(final_recommendations.filter(col("implementation_priority") == "High") \
...
summary = final_recommendations.groupBy("recommendation_category").agg(
...
).orderBy("recommendation_category").toPandas()

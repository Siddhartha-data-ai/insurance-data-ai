...
df_policy_analysis = df_policy_analysis.withColumn(
    ...
    when(col("annual_premium") > 0, 
         col("total_paid") / (col("annual_premium") * col("policy_age_days") / 365)).otherwise(0)
)
...
df_customer_value = df_policy_analysis.groupBy("customer_id").agg(
    ...
    df_customers.select("customer_id", "customer_tenure_months", "age_years", "credit_tier"),
    "customer_id",
    "left"
)
...
df_recommendations = df_policy_analysis.withColumn(
...
summary_category = final_recommendations.groupBy("recommendation_category").agg(

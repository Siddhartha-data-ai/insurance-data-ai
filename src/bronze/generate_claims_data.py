...
# Load policies
df_policies = spark.table(f"{catalog_name}.policies.policy_raw").filter(
    "policy_status IN ('Active', 'Cancelled', 'Lapsed')"
)
...
df_policies_indexed = df_policies.withColumn("policy_row", F.monotonically_increasing_id()).select(
    "policy_row",
    "policy_id",
    "customer_id",
    "policy_type",
...
df_claims = (
    df_base.withColumn("policy_row", (F.rand() * num_policies).cast("int"))
...
df_claims = df_claims.withColumn(
    "claim_type",
    when(
        F.col("policy_type") == "Auto",
        F.element_at(F.array([F.lit("Auto"), F.lit("Liability")]), when(F.rand() < 0.70, 1).otherwise(2)),
    )
...
df_claims = df_claims.withColumn(
    ...
    when(
        ...
        F.element_at(
            F.array([F.lit("Accident"), F.lit("Theft"), F.lit("Vandalism")]),
            when(F.rand() < 0.70, 1).when(F.rand() < 0.90, 2).otherwise(3),
        ),
    )
    .when(
        F.col("claim_type") == "Property",
        F.element_at(
            F.array([F.lit("Fire"), F.lit("Flood"), F.lit("Theft"), F.lit("Storm")]), ((F.rand() * 4).cast("int") + 1)
        ),
    )
    .when(
        F.col("claim_type") == "Health",
        F.element_at(
            F.array([F.lit("Medical"), F.lit("Hospital"), F.lit("Surgical")]), ((F.rand() * 3).cast("int") + 1)
        ),
    )
...
df_claims = (
    df_claims.withColumn("days_into_policy", (F.datediff(F.col("expiration_date"), F.col("effective_date")) * F.rand()))
    .withColumn("loss_date", F.expr("effective_date + CAST(days_into_policy AS INT)"))
    .drop("days_into_policy")
)

# Report date (0-30 days after loss, fraud cases report later)
df_claims = (
    df_claims.withColumn("is_potential_fraud", F.rand() < 0.05)
    .withColumn(
        "days_to_report",
        when(F.col("is_potential_fraud"), ((F.rand() * 20).cast("int") + 10)).otherwise(
            ((F.rand() * 5).cast("int") + 1)
        ),
    )
...
df_claims = df_claims.withColumn(
    "claim_status",
    F.element_at(
        F.array([F.lit("Reported"), F.lit("Under Investigation"), F.lit("Approved"), F.lit("Denied"), F.lit("Closed")]),
        when(F.rand() < 0.05, 1)
        .when(F.rand() < 0.15, 2)
        .when(F.rand() < 0.25, 3)
        .when(F.rand() < 0.30, 4)
...
df_claims = df_claims.withColumn(
    "closed_date",
    when(F.col("claim_status") == "Closed", F.expr("report_date + CAST(rand() * 90 + 10 AS INT)")).otherwise(
        F.lit(None)
    ),
)
...
df_claims = (
    ...
    .withColumn(
        "claimed_amount",
        when(F.col("is_potential_fraud"), F.col("claimed_amount_base") * 1.5).otherwise(F.col("claimed_amount_base")),
    )
...
df_claims = (
    df_claims.withColumn(
        "estimated_loss_amount", (F.col("claimed_amount") * (F.rand() * 0.2 + 0.90)).cast("decimal(15,2)")
    )
    .withColumn("reserved_amount", (F.col("estimated_loss_amount") * (F.rand() * 0.15 + 1.0)).cast("decimal(15,2)"))
    .withColumn(
        "paid_amount",
        when(
            F.col("claim_status").isin(["Approved", "Closed"]),
            F.least(F.col("claimed_amount") - F.col("deductible_amount"), F.col("coverage_amount") * 0.8),
        ).otherwise(F.lit(0)),
    )
...
df_claims = (
    df_claims.withColumn(
        "fraud_score", when(F.col("is_potential_fraud"), (F.rand() * 30 + 65)).otherwise((F.rand() * 40 + 10))
    )
...
# Assign adjusters
df_claims = df_claims.withColumn(
    "assigned_adjuster_id", F.concat(F.lit("ADJ"), F.lpad(((F.rand() * 300).cast("int") + 1), 4, "0"))
)
...
df_claims = df_claims.withColumn(
    ...
    F.element_at(
        F.array([F.lit("Self"), F.lit("Spouse"), F.lit("Child"), F.lit("Third Party")]),
        when(F.rand() < 0.60, 1).when(F.rand() < 0.80, 2).when(F.rand() < 0.90, 3).otherwise(4),
    ),
)
...
df_claims = df_claims.withColumn(
    "loss_location_state",
    when(F.rand() < 0.95, F.col("state_code")).otherwise(
        F.element_at(F.array([F.lit("CA"), F.lit("TX"), F.lit("FL"), F.lit("NY")]), ((F.rand() * 4).cast("int") + 1))
    ),
)
...
df_claims = df_claims.withColumn(
    "police_report_filed",
    when(F.col("loss_type").isin(["Accident", "Theft", "Vandalism"]), F.rand() < 0.80).otherwise(F.rand() < 0.20),
).withColumn(
    "police_report_number",
    when(
        F.col("police_report_filed"), F.concat(F.lit("PR"), F.lpad((F.rand() * 999999).cast("int"), 6, "0"))
...
df_claims = df_claims.withColumn(
    "days_open",
    when(F.col("claim_status") == "Closed", F.datediff(F.col("closed_date"), F.col("report_date"))).otherwise(
        F.datediff(F.current_date(), F.col("report_date"))
    ),
)

# Source metadata
df_claims = (
    df_claims.withColumn("source_system", F.lit("ClaimsManagement"))

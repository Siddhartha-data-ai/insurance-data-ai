...
df_base = (
    ...
    .withColumn("policy_number", concat(lit("P"), lpad(expr("cast(rand() * 9999999 as int)"), 7, "0")))
)

# Assign customers (some get multiple policies)
df_base = df_base.withColumn("customer_row", expr(f"cast(rand() * {num_customers} as int)"))

# Join with customers to get customer_id
df_customers_indexed = df_customers.withColumn("customer_row", F.monotonically_increasing_id())
...
df_policies = (
    df_policies.withColumn("policy_type_rand", expr("rand()"))
    .withColumn(
        "policy_type",
...
df_policies = df_policies.withColumn(
    "product_line",
    when(
        col("policy_type") == "Auto",
        element_at(array([lit("Personal Auto"), lit("Commercial Auto")]), when(expr("rand()") < 0.85, 1).otherwise(2)),
    )
    .when(
        col("policy_type") == "Home",
        element_at(
            array([lit("Homeowners"), lit("Renters"), lit("Condo")]),
            when(expr("rand()") < 0.60, 1).when(expr("rand()") < 0.90, 2).otherwise(3),
        ),
    )
    .when(
        col("policy_type") == "Life",
        element_at(
            array([lit("Term Life"), lit("Whole Life"), lit("Universal Life")]),
            when(expr("rand()") < 0.60, 1).when(expr("rand()") < 0.85, 2).otherwise(3),
        ),
    )
    .when(
        col("policy_type") == "Health",
        element_at(
            array([lit("Individual Health"), lit("Family Health")]), when(expr("rand()") < 0.45, 1).otherwise(2)
        ),
    )
    .otherwise(
        element_at(
            array([lit("Business Owners Policy"), lit("General Liability"), lit("Commercial Property")]),
...
df_policies = df_policies.withColumn(
    "product_code",
    concat(
        F.upper(F.substring(col("policy_type"), 1, 2)), lit("-"), lpad(expr("cast(rand() * 999 as int) + 1"), 3, "0")
    ),
)

# Policy status - realistic distribution
df_policies = (
    df_policies.withColumn("policy_status_rand", expr("rand()"))
    .withColumn(
        "policy_status",
...
df_policies = (
    df_policies.withColumn("application_days_ago", expr("cast(rand() * {} as int)".format(365 * 5 - 30)))
...
df_policies = df_policies.withColumn(
    "issue_date", date_add(col("application_date"), expr("cast(rand() * 10 as int) + 5"))
)

# Effective date: 0-30 days after issue
df_policies = df_policies.withColumn("effective_date", date_add(col("issue_date"), expr("cast(rand() * 30 as int)")))

# Bind date: same as or 1-2 days before effective date
df_policies = df_policies.withColumn("bind_date", date_add(col("effective_date"), -expr("cast(rand() * 2 as int)")))

# Policy term (6, 12, or 24 months typically)
df_policies = df_policies.withColumn(
    "policy_term_months",
    when(
        col("policy_type").isin(["Auto", "Home"]),
        element_at(array([lit(6), lit(12)]), when(expr("rand()") < 0.75, 2).otherwise(1)),
    )
    .when(
        col("policy_type") == "Life",
        element_at(
            array([lit(12), lit(120), lit(240)]),
            when(expr("rand()") < 0.50, 1).when(expr("rand()") < 0.85, 2).otherwise(3),
        ),
    )
    .otherwise(lit(12)),
)
...
# Expiration date
df_policies = df_policies.withColumn("expiration_date", expr("add_months(effective_date, policy_term_months)"))

# Cancellation date for cancelled/lapsed policies
df_policies = df_policies.withColumn(
    "cancellation_date",
    when(
        col("policy_status").isin(["Cancelled", "Lapsed"]),
        date_add(col("effective_date"), expr("cast(rand() * datediff(expiration_date, effective_date) as int)")),
    ).otherwise(lit(None)),
).withColumn(
    "cancellation_reason",
    when(
        ...
        element_at(
            array(
                [
                    lit("Non-Payment"),
                    lit("Customer Request"),
                    lit("Underwriting"),
                    lit("Fraud"),
                    lit("Loss of Interest"),
                    lit("Found Better Rate"),
                ]
            ),
...
df_policies = (
    df_policies.withColumn("base_premium_auto", expr("rand() * 1500 + 600"))
    ...
    .drop(
        "base_premium_auto", "base_premium_home", "base_premium_life", "base_premium_health", "base_premium_commercial"
    )
)
...
df_policies = df_policies.withColumn(
    "payment_frequency",
    element_at(
        array([lit("Annual"), lit("Semi-Annual"), lit("Quarterly"), lit("Monthly")]),
        when(expr("rand()") < 0.15, 1).when(expr("rand()") < 0.30, 2).when(expr("rand()") < 0.50, 3).otherwise(4),
    ),
)
...
df_policies = df_policies.withColumn(
    ...
    when(
        ...
        element_at(array([lit("Current"), lit("Overdue")]), when(expr("rand()") < 0.90, 1).otherwise(2)),
    )
...
df_policies = df_policies.withColumn(
    ...
    when(
        col("policy_type") == "Auto",
        element_at(
            array([lit(50000), lit(100000), lit(250000), lit(500000)]),
            when(expr("rand()") < 0.30, 1).when(expr("rand()") < 0.60, 2).when(expr("rand()") < 0.90, 3).otherwise(4),
        ),
    )
    .when(col("policy_type") == "Home", expr("cast(rand() * 700000 + 200000 as decimal(15,2))"))
    .when(
        col("policy_type") == "Life",
        element_at(
            array([lit(100000), lit(250000), lit(500000), lit(1000000), lit(2000000)]),
    ...
    .when(
        col("policy_type") == "Health",
        element_at(array([lit(50000), lit(100000), lit(250000), lit(1000000)]), expr("cast(rand() * 4 as int) + 1")),
    )
...
df_policies = df_policies.withColumn(
    ...
    when(
        col("policy_type") == "Auto",
        element_at(array([lit(250), lit(500), lit(1000), lit(2500)]), expr("cast(rand() * 4 as int) + 1")),
    )
    .when(
        col("policy_type") == "Home",
        element_at(array([lit(500), lit(1000), lit(2500), lit(5000)]), expr("cast(rand() * 4 as int) + 1")),
    )
    .when(
        col("policy_type") == "Health",
        element_at(array([lit(1000), lit(2500), lit(5000), lit(10000)]), expr("cast(rand() * 4 as int) + 1")),
    )
    .otherwise(element_at(array([lit(1000), lit(2500), lit(5000)]), expr("cast(rand() * 3 as int) + 1"))),
)

# Coverage dates (same as policy dates for simplicity)
df_policies = df_policies.withColumn("coverage_start_date", col("effective_date")).withColumn(
    "coverage_end_date", col("expiration_date")
)

# Assign agents
df_policies = df_policies.withColumn(
    "writing_agent_id", concat(lit("AGT"), lpad(expr("cast(rand() * 5000 as int) + 1"), 6, "0"))
).withColumn(
    "servicing_agent_id",
    when(expr("rand()") < 0.80, col("writing_agent_id")).otherwise(
        concat(lit("AGT"), lpad(expr("cast(rand() * 5000 as int) + 1"), 6, "0"))
    ),
)

# Agency and territory
df_policies = (
    df_policies.withColumn("agency_code", concat(lit("AGY"), lpad(expr("cast(rand() * 500 as int) + 1"), 4, "0")))
    .withColumn(
        "territory_code", concat(col("state_code"), lit("-"), lpad(expr("cast(rand() * 99 as int) + 1"), 2, "0"))
    )
    .withColumn(
        "county",
        element_at(
            array([lit("County A"), lit("County B"), lit("County C"), lit("County D"), lit("County E")]),
...
df_policies = df_policies.withColumn(
    "underwriter_id", concat(lit("UW"), lpad(expr("cast(rand() * 200 as int) + 1"), 4, "0"))
).withColumn(
    "underwriting_tier",
    when(col("credit_score") > 750, "Preferred").when(col("credit_score") > 650, "Standard").otherwise("Non-Standard"),
)
...
df_policies = df_policies.withColumn(
    "risk_class",
    when(
        col("underwriting_tier") == "Preferred",
        element_at(array([lit("A+"), lit("A"), lit("A-")]), expr("cast(rand() * 3 as int) + 1")),
    )
    .when(
        col("underwriting_tier") == "Standard",
...
df_policies = (
    df_policies.withColumn(
        "renewal_count", when(col("policy_status") == "Active", expr("cast(rand() * 5 as int)")).otherwise(0)
    )
    .withColumn("is_renewal", col("renewal_count") > 0)
    .withColumn(
        "parent_policy_id",
        when(col("is_renewal"), concat(lit("POL"), lpad(expr("cast(rand() * 1000000 as int)"), 10, "0"))).otherwise(
            lit(None)
        ),
    )
)
...
df_policies = (
    df_policies.withColumn("source_system", lit("PolicyAdmin"))

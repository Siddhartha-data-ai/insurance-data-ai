...
df_customers = df_base \
    .withColumn("first_name", 
                when(F.rand() < 0.5, 
                     element_at(array([lit(x) for x in first_names_male]), 
                               ((F.rand() * len(first_names_male)).cast("int") + 1)))
                .otherwise(
                     element_at(array([lit(x) for x in first_names_female]), 
                               ((F.rand() * len(first_names_female)).cast("int") + 1)))) \
    .withColumn("last_name", 
                element_at(array([lit(x) for x in last_names]), 
                          ((F.rand() * len(last_names)).cast("int") + 1))) \
    .withColumn("gender", when(F.rand() < 0.5, "M").otherwise("F"))

# Generate date of birth (ages 18-85)
df_customers = df_customers \
    .withColumn("days_old", ((F.rand() * (365 * 67)).cast("int") + (365 * 18))) \
...
df_customers = df_customers \
    .withColumn("email", concat(
        F.lower(col("first_name")), lit("."), F.lower(col("last_name")),
        lit("@"),
        element_at(array([lit("gmail.com"), lit("yahoo.com"), lit("hotmail.com"), 
                         lit("outlook.com"), lit("aol.com"), lit("icloud.com")]),
                  ((F.rand() * 6).cast("int") + 1))
    )) \
...
df_customers = df_customers \
    .withColumn("state_code", 
                element_at(array([lit(x) for x in state_codes]), 
                          ((F.rand() * len(state_codes)).cast("int") + 1)))

# Generate address
df_customers = df_customers \
    .withColumn("address_line1", concat(
        ((F.rand() * 9999 + 1).cast("int")),
        lit(" "),
        element_at(array([lit("Main"), lit("Oak"), lit("Pine"), lit("Maple"), lit("Cedar"),
                         lit("Elm"), lit("Washington"), lit("Lake"), lit("Hill"), lit("Park")]),
                  ((F.rand() * 10).cast("int") + 1)),
        lit(" "),
        element_at(array([lit("St"), lit("Ave"), lit("Blvd"), lit("Dr"), lit("Ln"), lit("Rd")]),
                  ((F.rand() * 6).cast("int") + 1))
    )) \
    .withColumn("address_line2", 
                when(F.rand() < 0.3, 
                     concat(lit("Apt "), ((F.rand() * 999 + 1).cast("int"))))
                .otherwise(lit(None))) \
    .withColumn("city", 
                element_at(array([
                    lit("Springfield"), lit("Franklin"), lit("Clinton"), lit("Madison"),
                    lit("Georgetown"), lit("Arlington"), lit("Salem"), lit("Fairview"),
                    lit("Riverside"), lit("Oak Grove"), lit("Hillside"), lit("Lakewood")
...
df_customers = df_customers \
    .withColumn("customer_type", 
                element_at(array([lit("Individual"), lit("Family"), lit("Business")]),
                          when(F.rand() < 0.6, 1)
                          .when(F.rand() < 0.9, 2)
                          .otherwise(3))) \
    .withColumn("marital_status",
                element_at(array([lit("Single"), lit("Married"), lit("Divorced"), lit("Widowed")]),
                          when(F.rand() < 0.35, 1)
                          .when(F.rand() < 0.85, 2)
                          .when(F.rand() < 0.95, 3)
                          .otherwise(4)))
...
df_customers = df_customers \
    .withColumn("credit_score_raw", (F.rand() + F.rand() + F.rand() + F.rand() + F.rand() + F.rand()) / 6.0) \
    .withColumn("credit_score", 
                F.expr("300 + credit_score_raw * 550").cast("int")) \
    .drop("credit_score_raw")

# Generate occupation
occupations = [
    "Software Engineer", "Teacher", "Nurse", "Sales Manager", "Accountant",
    "Retired", "Business Owner", "Consultant", "Engineer", "Doctor",
    "Attorney", "Manager", "Analyst", "Technician", "Administrator",
    "Driver", "Mechanic", "Electrician", "Plumber", "Carpenter",
    "Student", "Unemployed", "Self Employed", "Executive", "Professor"
]
df_customers = df_customers \
    .withColumn("occupation",
                element_at(array([lit(x) for x in occupations]),
                          ((F.rand() * len(occupations)).cast("int") + 1)))
...
df_customers = df_customers \
    .withColumn("annual_income",
                ...
                .when(col("occupation").isin(["Doctor", "Attorney", "Executive"]), 
                      ((F.rand() * 200000 + 100000).cast("decimal(15,2)")))
                .when(col("occupation").isin(["Software Engineer", "Engineer", "Manager"]), 
...
df_customers = df_customers \
    .withColumn("customer_since_days", (F.rand() * (365 * 10)).cast("int")) \
    ...
    .withColumn("customer_status",
                when(F.rand() < 0.92, "Active")
                .when(F.rand() < 0.97, "Inactive")
                .otherwise("Cancelled"))

# Communication preferences
df_customers = df_customers \
    .withColumn("communication_preference",
                element_at(array([lit("Email"), lit("Phone"), lit("Text"), lit("Mail")]),
                          ((F.rand() * 4).cast("int") + 1))) \
    .withColumn("preferred_language", 
                when(F.rand() < 0.85, "English")
                .when(F.rand() < 0.95, "Spanish")
                .otherwise("Other")) \
...
final_columns = [
    ...
    "address_line1", "address_line2", "city", "state_code", "zip_code", "country",
    "customer_type", "credit_score", "marital_status", "occupation", "annual_income",
    "communication_preference", "preferred_language", "marketing_opt_in", "customer_segment",
    "customer_since_date", "customer_status", "assigned_agent_id", "assigned_region",
    "risk_profile", "source_system", "source_record_id", "ingestion_timestamp",
    "record_hash", "data_lineage"
]

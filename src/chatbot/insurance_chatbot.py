...
def create_forecast_chart(df):
    ...
    if not df_all.empty and "forecast_date" in df_all.columns:
        ...
        fig.update_layout(
            title="Claim Volume Forecast", xaxis_title="Date", yaxis_title="Predicted Claims", hovermode="x unified"
        )
...
def generate_response(user_input):
    ...
    if intent == "churn":
        ...
        if df is not None and not df.empty:
            ...
            text += (
                "The table below shows detailed customer information. These customers need immediate retention efforts."
            )
        else:
        ...
        if df is not None and not df.empty:
            days = params.get("days", 30)
            total_predicted = (
                df[df["claim_type"] == "ALL_TYPES"]["predicted_claims"].sum()
                if "claim_type" in df.columns
                else df["predicted_claims"].sum()
            )
            total_amount = (
                df[df["claim_type"] == "ALL_TYPES"]["predicted_amount"].sum()
                if "claim_type" in df.columns
                else df["predicted_amount"].sum()
            )
    ...
    return {"text": text, "data": df, "chart": chart}
    ...
    with st.sidebar:
        ...
        if st.button("💰 Pricing Opportunities"):
            st.session_state.messages.append(
                {"role": "user", "content": "Show me high priority pricing recommendations"}
            )

...
dbutils.widgets.dropdown("quick_action", "None", 
                         ["None", "📊 Executive Summary", "🔴 High Risk Customers", 
                          "🚨 Critical Fraud Cases", "📈 30-Day Forecast", 
...
def create_suggestions_html(suggestions):
    ...
    buttons_html = "".join([f'<li style="margin: 10px 0; padding: 10px; background: #e8f4f8; border-radius: 5px; cursor: pointer;">💡 {s}</li>' 
...
def generate_response(user_input, gold_catalog, silver_catalog, environment):
    ...
    try:
        ...
        return {
            'html': create_response_html("Query Error", f"Error executing query: {str(e)}"),
            'suggestions': []
        }
    ...
    return {
        'html': create_response_html("Results", f"Found {len(df):,} results"),
        'data': df
    }
